...
public class Connect
{
	public static boolean isConnected()
	{
		//TODO check aast id
		try{
			File file=new File(Environment.getExternalStorageDirectory().getPath()+"/connect.json");
			return file.exists();
	...
	public static JSONObject getData()// throws Exception
	{
		try{
			...
			if(file.exists())
			{
				FileInputStream in=new FileInputStream(Environment.getExternalStorageDirectory().getPath()+"/connect.json");
...
//		return new JSONObject();
	...
	{
		FileOutputStream out=new FileOutputStream(Environment.getExternalStorageDirectory().getPath()+"/connect.json");
...
// Response Handler for "https://graph.facebook.com/me?access_token="; happens after ServerRequestHandler receives SERVICE?access_token=ACCESS_TOKEN
class facebook implements URLThread_CallBack
{
	...
	public void URLCallBack(String response)
	{
		...
		try{
			...
			//try updating user's data on the server using Connect.json data
...
class prifileCallback implements URLThread_CallBack
{
	@Override
	public void URLCallBack(String response)
	{
		try{
			...
			//try updating user's data on the server using Connect.txt data
				new URLThread("yoga1290.appspot.com/schoolmate/student?id="+res.getString("id")+"&pin=1"+res.getString("pin"), 
						new URLThread_CallBack() {
							@Override
							public void URLCallBack(String response)
							{
